...
from navigation.configs.config import Config
...
def main():
	...
	shutil.copytree('navigation/configs', os.path.join(config.training.output_dir, 'configs'))
	...
	if config.training.cuda and torch.cuda.is_available():
		...
		else:		# if you normalized the observation, you may also want to save rms
			# torch.save([
			# 	actor_critic,
